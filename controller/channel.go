...
func getBatchCreateDedupKey(channel *model.Channel) string {
	baseURL := ""
	if channel.BaseURL != nil {
		baseURL = strings.TrimSpace(*channel.BaseURL)
	}
	return baseURL + "\x00" + strings.TrimSpace(channel.Key)
}

func countNonEmptyChannelKeys(channel *model.Channel) int {
	count := 0
...
func normalizeBatchCreateChannel(channel *model.Channel, createdTime int64) error {
	channel.Id = 0
	...
	case common.ChannelStatusUnknown, common.ChannelStatusAutoDisabled:
		channel.Status = common.ChannelStatusEnabled
	case common.ChannelStatusEnabled, common.ChannelStatusManuallyDisabled:
	default:
		return fmt.Errorf("status must be 1 (enabled) or 2 (disabled)")
	}
	...
	keyCount := countNonEmptyChannelKeys(channel)
	if keyCount == 0 {
		return fmt.Errorf("multi-key channel must include at least one key")
	}
	...
	default:
		return fmt.Errorf("multi-key mode must be random or polling")
	}
	return nil
}
...
func BatchCreateChannels(c *gin.Context) {
	...
	if err := common.DecodeJson(c.Request.Body, &channels); err != nil {
		common.ApiError(c, err)
		return
	}
	if len(channels) == 0 {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "channels cannot be empty",
		})
		return
	}
	if len(channels) > batchCreateChannelsLimit {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": fmt.Sprintf("a maximum of %d channels can be created at once", batchCreateChannelsLimit),
		})
		return
	}
	...
	dedupedChannels := make([]model.Channel, 0, len(channels))
	for i := range channels {
		if err := normalizeBatchCreateChannel(&channels[i], createdTime); err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": fmt.Sprintf("channels[%d]: %s", i+1, err.Error()),
			})
			return
		}
		if err := validateChannel(&channels[i], true); err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": fmt.Sprintf("channels[%d]: %s", i+1, err.Error()),
			})
		...
		dedupKey := getBatchCreateDedupKey(&channels[i])
		if _, ok := seenChannels[dedupKey]; ok {
			continue
		}
		seenChannels[dedupKey] = struct{}{}
		dedupedChannels = append(dedupedChannels, channels[i])
	}
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "",
		"data": BatchCreateChannelsResponse{
			CreatedCount: len(channels),
			Ids:          ids,
		},
	})
}
